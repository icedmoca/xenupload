...
return array (
  ...
  'option_explain.boardUrl' => 'The primary URL to your board. Do not include a trailing "/" or "index.php". The URL should look similar to this: http://www.example.com/forum',
...
This option requires that the specified bounce account is a catch-all account or supports a "+" as a wildcard separator (such as in Gmail). For example, if this option is enabled with a bounce address of bounce@example.com, the email might be returned to bounce+123abc+user=domain.com@example.com.',
  ...
  'option_explain.googleAnalyticsWebPropertyId' => 'You may enter your <a href="http://www.google.com/analytics/">Google Analytics</a> web property ID here to have the Analytics HTML automatically added to your public-facing pages.',
...
/*$(function()
{
  ...
  'option_explain.ncmsAllowDisplayGroupChanging' => 'Allow users to manually change their display group. If set to false, the highest priority group will always be forced upon the user.',
  'option_explain.ncmsAllowUnlinking' => 'Should users be allowed to unlink their account after linking it?',
  'option_explain.ncmsApiAccessIps' => 'List of IPs that can use the MineSync API, separated by commas (","). Leave empty for all IPs.',
  'option_explain.ncmsApiKey' => 'This key will authenticate a client with the API. Generate a new key here, and ensure the same key is set on the server.',
  'option_explain.ncmsAvatarSource' => '',
  'option_explain.ncmsDisableServerStatusCache' => 'This disables caching for the server status widgets. By default, they are updated every minute. If you disable caching, they will be updated on every page load where a server status widget is displayed.',
  'option_explain.ncmsEnableAvatarSync' => 'Automatically set user avatars to their Minecraft skin head if no custom avatar is set.',
  'option_explain.ncmsEnableLinking' => 'Are users allowed to link their account with the forums? If set to off, trying to link will show an error.',
  'option_explain.ncmsEnablePlayerShowcase' => 'Enable the player showcase block on messages and profiles',
  'option_explain.ncmsForceAvatarSync' => 'Force avatars for players with a linked Minecraft account to have their Minecraft avatars displayed. Requires the above option to be ticked.',
  'option_explain.ncmsKeyInvalidationTime' => 'Time in minutes that old keys will be invalidated. By default this is 6 hours (60 * 6 minutes)',
  'option_explain.ncmsNewLinkInvalidationTime' => 'Time in minutes for new links to become invalidated. New links are created to notify the server that a new user has linked their account with the website. These can be removed after a period of time. To disable, effectively set this to a very large number. Default is the maximum length possible (8171 years).',
  'option_explain.ncmsRegisterOnlyThroughServer' => 'Tick this to only allow players to register via the server with a link token.',
  ...
  'option_explain.th_instagramUrl' => '',
  'option_explain.th_linkedInUrl' => '',
